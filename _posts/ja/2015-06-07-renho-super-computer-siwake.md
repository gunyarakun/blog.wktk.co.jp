...
- 仕分け本番の前に事前ヒアリングが行われ、「世界一になる理由を説明できるようにしてください」というリクエストが被仕分け側に出されていた。
...
- 凍結は見直された。コストパフォーマンスに優れなくとも、10ペタクラスのスパコンそのものは必要性が高かった、ということなのだろう。
- 無理目のプロジェクトだと思われたのに、富士通は1社でプロジェクトを遂行した。これはすごい。スパコン「京」の誕生である。
...
- 「京」で使われたプロセッサは、SPARCというアーキテクチャに基づいていた。
  ...
  - ARMアーキテクチャに基づいたプロセッサは、消費電力の少なさが特徴で、iPhoneやNintendo 3DSなどにも用いられている。スカラ型。
