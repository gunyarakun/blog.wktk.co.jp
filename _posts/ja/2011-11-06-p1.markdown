...
<pre class="prettyprint lang-bash">sbaz install scala-tool-support
...
<pre class="prettyprint lang-bash">ack File</pre>
でgrep。src/main/scala/punk/PunkFilter.scalaにreadFile()というメソッドがあるらしい。
<pre class="prettyprint lang-scala">    private def readFile(url: URL) = {
...
<pre class="prettyprint lang-scala">import com.twitter.finagle.Service
...
<pre class="prettyprint lang-scala">response.setContent(copiedBuffer(scala.io.Source.fromFile("src/main/webapp/index.html").mkString, UTF_8))</pre>
...
<pre class="prettyprint lang-scala">response.setContent(copiedBuffer(scala.io.Source.fromFile("src/main/webapp" + request.getUri).mkString, UTF_8))</pre>
動作した。
...
<pre class="prettyprint lang-scala">  val tentenRegex = """\.\.""".r
...
<pre class="prettyprint lang-scala">  val path = request.getUri
